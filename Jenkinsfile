pipeline {
  ...
  // NOTE: removed `options { ... }` to avoid NoSuchMethodError on 'options'

  parameters {
    string(
      name: 'APP_REPO',
      defaultValue: 'jmiguelcheq/calculator-demo-jenkins',
      description: 'owner/repo to clone in LOCAL mode (when APP_SHA is set)'
    )
    string(
      name: 'APP_SHA',
      defaultValue: '',
      description: 'App commit SHA to test locally; leave empty to use CALC_URL (REMOTE mode)'
    )
    string(
      name: 'CALC_URL',
      defaultValue: 'https://jmiguelcheq.github.io/calculator-demo-jenkins',
      ...
      steps {
        sh '''
          ...
          if command -v find >/dev/null 2>&1; then
            find ci -type f -name "*.sh" -exec chmod +x {} \\; 2>/dev/null || true
          else
      ...
      steps {
        script {
          ...
          if (params.APP_SHA?.trim()) {
            ...
            env.APP_COMMIT = params.APP_SHA.trim()

            withEnv(["APP_REPO=${params.APP_REPO}", "APP_SHA=${params.APP_SHA.trim()}"]) {
              sh '''
                set -e
          ...
          if (!baseUrl) {
            error "BASE_URL is empty. Provide CALC_URL or APP_SHA."
          }
    ...
    stage('Build & Run Tests') {
      ...
      post {
        always {
          ...
          // Allure single-file + full report
          sh '''
            set +e
            mkdir -p target
  ...
  post {
    always {
      script {
        ...
        // prefer the app commit if provided, otherwise the testing-repo commit, otherwise "unknown"
        def commitForLogs = params.APP_SHA?.trim() ?: (env.GIT_COMMIT ?: 'unknown')
        ...
        withCredentials([
          ...
          withEnv([
            "TEST_RESULT=${resultVal}",
            "TEST_COMMIT=${commitForLogs}"
          ]) {
            sh '''
              set -eu
              ...
              ./ci/push_to_loki.sh || echo "⚠️ Loki summary push failed (non-blocking)."

              # Optional: push console tail as separate stream
              ...
              ./ci/push_to_loki.sh || echo "⚠️ Loki console push failed (non-blocking)."
            '''
          }
        }
